...
def test_provider_payload_reads_workspace_authority_and_selects_related_terms(tmp_path: Path) -> None:
    ...
    write_json(workspace.glossary_path, {"schema_version": "3.0", "terms": [
        {"source": "雨宮慶", "target": "雨宫庆", "category": "person", "status": "active", "evidence": [{"paragraph_id": "p1"}], "note": "secret"},
        {"source": "止血鉗", "target": "止血钳", "category": "medical_device", "status": "active", "evidence": [{"paragraph_id": "p1"}]},
        {"source": "手", "target": "手", "category": "body_part", "status": "retired", "evidence": [{"paragraph_id": "p1"}]},
        {"source": "無関係な場所", "target": "无关地点", "category": "location", "status": "active", "evidence": [{"paragraph_id": "p9"}]},
    ], "conflicts": [], "revisions": []})
