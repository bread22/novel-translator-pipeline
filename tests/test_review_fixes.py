...
from translator.web.routes import books, tasks
from translator.web.models import ParagraphUpdateRequest, RetranslateParagraphRequest
from translator.core.job_manager import JobManager
from translator.web.app import create_app


def manifest(path):
    write_json(path, {'book': 'fixture', 'title': 'fixture', 'chapters': [{'id': 'c1', 'paragraphs': [
        {'id': 'p1', 'source': '一', 'translated': ''},
        {'id': 'p2', 'source': '二', 'translated': ''},
    ]}]})


def test_translation_overwrites_concurrent_manual_edit(tmp_path, monkeypatch):
...
def test_dispatch_cancel_signal_is_lost(tmp_path, monkeypatch):
    ...
    with patch('translator.core.job_manager.threading.Thread', InterleavedThread), patch('translator.core.job_manager.broadcaster.broadcast_sync'):
        manager._dispatch()
...
def test_replacement_upload_keeps_old_source_and_state(tmp_path, monkeypatch):
    path = tmp_path / 'manifest.json'
    manifest(path)
    out = tmp_path / 'output'
...
def test_spa_can_serve_file_outside_dist(tmp_path, monkeypatch):
    dist = tmp_path / 'dist'
    dist.mkdir()
...
def test_provider_error_during_cancel_leaves_occupied_slot(tmp_path, monkeypatch):
    import threading
    ...
    class FailingPipeline:
        ...
        def is_chapter_completed(self, chapter): return False
