...
def cancel_queue_item(item_id: str) -> QueueStatusResponse:
    ...
    success = job_manager.cancel_item(item_id)
    if not success:
...
def retry_queue_item(item_id: str) -> QueueStatusResponse:
    """单键重新入队重试失败或取消的队列项"""
    res = job_manager.retry_item(item_id)
    if not res:
...
def move_queue_item(item_id: str, request: QueueItemMoveRequest) -> QueueStatusResponse:
    ...
    success = job_manager.move_item(item_id, request.direction)
    if not success:
...
def reorder_queue(request: QueueReorderRequest) -> QueueStatusResponse:
    """拖拽调序后批量原子更新待处理任务顺序"""
    job_manager.reorder(request.item_ids)
    return job_manager.get_status()
...
def clear_queue(request: QueueClearRequest) -> QueueStatusResponse:
    """清理已完成、失败或取消的历史队列项"""
    job_manager.clear(scope=request.scope)
    return job_manager.get_status()
